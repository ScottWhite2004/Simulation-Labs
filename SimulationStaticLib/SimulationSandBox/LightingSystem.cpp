...
void LightingSystem::createBuffer(const VulkanContext& ctx, VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& memory) const {
	...
	if (vkCreateBuffer(ctx.getDevice(), &info, nullptr, &buffer) != VK_SUCCESS)
	...
	alloc.memoryTypeIndex = findMemoryType(ctx.getPhysicalDevice(), req.memoryTypeBits, properties);

	if (vkAllocateMemory(ctx.getDevice(), &alloc, nullptr, &memory) != VK_SUCCESS)
		throw std::runtime_error("LightingSystem: vkAllocateMemory failed");

	vkBindBufferMemory(ctx.getDevice(), buffer, memory, 0);
}

void LightingSystem::create(const VulkanContext& ctx, uint32_t framesInFlight) {
	...
	for (uint32_t i = 0; i < framesInFlight; ++i)
	{
		...
		if (vkMapMemory(ctx.getDevice(), _memories[i], 0, size, 0, &_mapped[i]) != VK_SUCCESS || _mapped[i] == nullptr)
...
void LightingSystem::destroy(const VulkanContext& ctx) {
	for (uint32_t i = 0; i < _framesInFlight; ++i)
	{
		if (_mapped[i] != nullptr)
		{
			vkUnmapMemory(ctx.getDevice(), _memories[i]);
			_mapped[i] = nullptr;
		}
		if (_buffers[i] != VK_NULL_HANDLE)
		{
			vkDestroyBuffer(ctx.getDevice(), _buffers[i], nullptr);
			_buffers[i] = VK_NULL_HANDLE;
		}
		if (_memories[i] != VK_NULL_HANDLE)
		{
			vkFreeMemory(ctx.getDevice(), _memories[i], nullptr);
